...
function isValidLayout(value: unknown): value is Layout {
  return (
    typeof value === 'string' &&
    value in LAYOUTS &&
    (value === 'grid' || value === 'carousel' || value === 'list' || value === 'cards')
  )
}
  ...
  try {
    ...
    if (error instanceof DOMException) {
      ...
      if (error.name === 'QuotaExceededError') {
        console.warn('localStorage quota exceeded, cannot read layout preference')
...
function safeSetLocalStorage(key: string, value: string): boolean {
  ...
  try {
    ...
    if (error instanceof DOMException) {
      if (error.name === 'QuotaExceededError') {
        console.warn('localStorage quota exceeded, cannot save layout preference')
        ...
        console.warn('localStorage access denied (private browsing mode?), layout preference not saved')
      } else {
...
export function useLayout() {
  ...
  const changeLayout = (layout: Layout) => {
    ...
    if (isValidLayout(layout)) {
      ...
      console.error(`Invalid layout "${layout}" provided to changeLayout, ignoring`)
    }
  }
